...
! Constituent, tributary, and widthdrawal totals
  ...
  ALLOCATE (EXH2O(NWB),  BETA(NWB),   EXOM(NWB),   EXSS(NWB),   DXI(NWB),    CBHE(NWB),   TSED(NWB),   TSEDF(NWB),  FI(NWB), NTUINT(NWB)) !CSW 7/20/2021
  ...
  ALLOCATE (CN(NCT))
  ALLOCATE (SSS(NSS),    TAUCR(NSS),  SEDRC(NSS),  NTUISS(NSS))   !,  SSFLOC(NSS), FLOCEQN(NSS))   SR 04/21/13   !CSW 7/20/2021
  ALLOCATE (CDSUM(NDC))
  ALLOCATE (DTRC(NBR))
  ...
  ALLOCATE (AC(NAL),     ASI(NAL),    ACHLA(NAL),  AHSP(NAL),   AHSN(NAL),   AHSSI(NAL),  NTUALG(NAL))    !CSW 7/20/2021
 ...
 ! BIOENERGETICS !mlm
  ...
  READ(1222,'(//(8X,A8,2I8))') BIOC,NBIO,NIBIO
  ALLOCATE (BIOEXPFN(NIBIO),WEIGHTNUM(NIBIO),C2ZOO(KMX,IMX,NCT),VOLROOS(IMX),C2W(IMX,NCT+2),IBIO(NIBIO))
  C2W = 0.0
  READ(1222,'(//(:8X,9F8.0))') (BIOD(II),II=1,NBIO)
  READ(1222,'(//(:8X,9F8.0))') (BIOF(II),II=1,NBIO)
  READ(1222,'(//(:8X,9I8))') (IBIO(II),II=1,NIBIO)
  read(1222,'(//(8x,a72))') biofn
  close(1222)
...
! Derived variables
  ...
  CBODU => CD(:,:,19); PH   => CD(:,:,20); CO2  => CD(:,:,21); HCO3 => CD(:,:,22); CO3   => CD(:,:,23); NTU  => CD(:,:,24) ! CSW 7/20/2021
...
! Kinetics coefficients
  ...
  READ (CON,'(//(8X,4F8.0,2A8,F8.0))') (EXH2O(JW),  EXSS(JW),   EXOM(JW),   BETA(JW),   EXC(JW),   EXIC(JW), NTUINT(JW), JW=1,NWB)
  ...
  READ (CON,'(//(8X,F8.0,A,2F8.0))')  (SSS(JS),    SEDRC(JS),  TAUCR(JS),  NTUISS(JS),                         JS=1,NSS)     !CSW 7/20/2021  ! READ (CON,'(//(8X,F8.0,A8,2F8.0,I8))') (SSS(JS), SEDRC(JS),  TAUCR(JS),  SSFLOC(JS), FLOCEQN(JS),            JS=1,NSS) !SR 04/21/13
  ...
  READ (CON,'(//(8X,6F8.0,I8,2F8.0))') (AP(JA),     AN(JA),     AC(JA),     ASI(JA),    ACHLA(JA), APOM(JA),                        &
                                       ANEQN(JA),  ANPR(JA), NTUALG(JA),                                       JA=1,NAL)    !CSW 7/20/2021
