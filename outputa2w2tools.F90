SUBROUTINE OUTPUTA  
  ...
  INTEGER :: JAD,JAF,IFLAG,JWWD,NLINES,ITOT,JJ, NUMOUTLETS,JSSS(100),JJC, IC
  REAL    :: TSUM,QSUMM,CGASD,XDUM,QOUTLET(100),TOUTLET(100)  
...
! code for error checking
  ...
  IF (BIOEXP) THEN  
    ...
    !    IF (JDAY.GE.NXBIO) then .OR.JDAY.GE.BIOD(BIODP+1)) THEN  
    IF (JDAY.GE.NXTBIO) THEN  
      ! IF (JDAY.GE.BIOD(biodp)) THEN  ! can't get multiple output dates to run:  keeps telling me biod(biodp) needs to be a scalar ?? mlm  
      ! BIODP  = biodp+1  
      ! NXBIO = BIOD(1) !mlm3  
      !nxbio = biod(2)  
      ! END IF  
      !NXTBIO = NXTBIO+BIOF(biodp)  
      ...
      DO J=1,NIBIO  
        ...
        DO K = KTWB(JW),KBI(I)  
          ...
          END DO  
	WRITE(BIOEXPFN(J),'(F8.2,I8,3F8.2,<NZOOE-NZOOS+1>F8.3,I8,2F8.2,A,X,I0,A,I0)') JDAY,IBIO(J),DEPTHM(K,I),T1(K,I),GAMMA(K,I),&
        ...
        DO K=KTWB(JW),KLIM  
          ...
          END DO  
			     C2W(I,NAC+1) = C2W(I,NAC+1)+CD(K,I,20)*BH(K,I) !PH
        ...
        WRITE(WEIGHTNUM(J),'(F10.3,<NAC>(1X,F9.3),2(1X,F9.3))')JDAY,(C2W(I,JJC),JJC = 1,NAC+2)  
        VOLROOS = 0.0
    ...
    IF (JDAY.GE.NXTMTS.OR.JDAY.GE.TSRD(TSRDP+1)) THEN 
      ...
      DO J=1,NIKTSR  
        ...
        IF (ICE_COMPUTATION) THEN  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,17F10.2,1000A)') JDAY,DLT,ELWS(I),T1(K,I),U(K,I),QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),ICETH(I),rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i),(ADJUSTR(C2CH(JAC)),JAC=1,NAC),                      &  ! CB 7/26/07  !CSW 7/21/2021
        ...
        ELSE  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,16F10.2,1000A)') JDAY,DLT,ELWS(I),T1(K,I),U(K,I),QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i),(ADJUSTR(C2CH(JAC)),JAC=1,NAC),(ADJUSTR(EPCH(JE)),            &  ! CB 7/26/07  !CSW 7/21/2021
...
9864      FORMAT('ZONE T="',f9.3,'"',' I=',I3,' J=',I3,' F=POINT')  
          DO JB=BS(JW),BE(JW)  
            DO I=CUS(JB), DS(JB)+1  
              ...
              IF(I /= DS(JB)+1)THEN  
                IF(HABTATC  == '      ON')THEN
                  WRITE (CPL(JW),9999) X1(I),ELWS(I),U(K,I),-W(K,I),T1(K,I),RHO(K,I),HAB(K,I),(C2(K,I,CN(JC)),JC=1,NAC)  
                ELSE
              ...
              ELSE  
                ...
                WRITE (CPL(JW),9999) X1(I),ELWS(I),XDUM,XDUM,XDUM,XDUM,XDUM,(XDUM, JJ=1,NAC)  
                ELSE                                                                           ! SW 7/15/14
                WRITE (CPL(JW),9999) X1(I),ELWS(I),XDUM,XDUM,XDUM,XDUM,(XDUM, JJ=1,NAC)        ! SW 7/15/14
                ENDIF                                                                          ! SW 7/15/14
              ENDIF  
              DO K=KTWB(JW),KMX-1  
                IF(I /= DS(JB)+1 .AND. K <= KB(I))THEN  
                IF(HABTATC  == '      ON')THEN
                  WRITE (CPL(JW),9999) X1(I),ELWS(I)-DEPTHM(K,I),U(K,I),-W(K,I),T1(K,I),RHO(K,I),HAB(K,I),(C2(K,I,CN(JC)),JC=1,NAC) 
                ELSE
                ...
                ENDIF
                  ...
                  IF(K == KB(I))THEN  
                      IF(HABTATC  == '      ON')THEN
                    WRITE (CPL(JW),9999) X1(I),ELWS(I)-DEPTHB(K,I),U(K,I), -W(K,I),T1(K,I),RHO(K,I),HAB(K,I),(C2(K,I,CN(JC)),JC=1,NAC)  
                      ELSE
                ...
                ELSE  
                  XDUM=-99.0  
                  IF(HABTATC  == '      ON')THEN
                  WRITE (CPL(JW),9999) X1(I),ELWS(I-1)-DEPTHM(K,I-1),XDUM,XDUM,XDUM,XDUM,XDUM,(XDUM, JJ=1,NAC) 
                  ELSE
                  WRITE (CPL(JW),9999) X1(I),ELWS(I-1)-DEPTHM(K,I-1),XDUM,XDUM,XDUM,XDUM,(XDUM, JJ=1,NAC)      ! SW 7/15/14
                  ENDIF
                  
                  IF(K == KB(I))THEN 
                    IF(HABTATC  == '      ON')THEN
                    WRITE (CPL(JW),9999) X1(I),ELWS(I-1)-DEPTHB(K,I-1),XDUM,XDUM,XDUM,XDUM,XDUM,(XDUM, JJ=1,NAC)  
                    ELSE
                    WRITE (CPL(JW),9999) X1(I),ELWS(I-1)-DEPTHB(K,I-1),XDUM,XDUM,XDUM,XDUM,(XDUM, JJ=1,NAC)         ! SW 7/15/14
                    ENDIF
          ...
          DO JB=BS(JW),BE(JW)  
            IF (DS(JB) == IWDO(J)) THEN  
              ...
              DO JS=1,NSTR(JB)  
                ...
                WRITE(WDO2(JFILE,1),'(F8.2,F8.3)')JDAY,QSTR(JS,JB)  
                WRITE(WDO2(JFILE,2),'(F8.2,F8.2)')JDAY,TAVG(JS,JB)  
                IF (CONSTITUENTS)then
                  cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qstr(js,jb)*cavg(js,jb,cn(1:nac))                 ! cb 1/16/13
                  WRITE (WDO2(JFILE,3),'(F8.2,1000F8.3)') JDAY,(CAVG(JS,JB,CN(JC)),     JC=1,NAC)  
                end if
                IF (DERIVED_CALC)then
                  cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qstr(js,jb)*cdavg(js,jb,cdn(1:nacd(jw),jw))                 ! cb 1/16/13 
                  WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVG(JS,JB,CDN(JD,JW)),JD=1,NACD(JW))  
                end if
              ENDDO  
              JSSS(JB)=NSTR(JB)  
            END IF  
   ...
   !     JWD=0                        ! sw 5/17/13
        ...
        DO JJ=1,NWD  
          ...
          IF (IWD(JWD) == IWDO(J)) THEN  
            ...
            WRITE(WDO2(JFILE,1),'(F8.2,F8.3)')JDAY,QWD(JWD)  
            WRITE(WDO2(JFILE,2),'(F8.2,F8.2)')JDAY,TAVGW(JWD)  
            IF (CONSTITUENTS)then
              cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qwd(jwd)*cavgw(jwd,cn(1:nac))                 ! cb 1/16/13  
              WRITE (WDO2(JFILE,3),'(F8.2,1000F8.3)') JDAY,(CAVGW(JWD,CN(JC)),     JC=1,NAC)  
            end if
            IF (DERIVED_CALC)THEN  
              !DO JW=1,NWB  
              !  IF (IWDO(J) >= US(BS(JW)) .AND. IWDO(J) <= DS(BE(JW))) EXIT  
              ...
              WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVGW(JWD,CDN(JD,JW)),JD=1,NACD(JW))  
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NSP  ! spillways
          ...
          IF(IWDO(J) == IUSP(JS))THEN  
            JFILE=JFILE+1  
            WRITE(WDO2(JFILE,1),'(F8.2,F8.3)')JDAY,QSP(JS)  
            IF(LATERAL_SPILLWAY(JS))THEN  
            !  JWD=JWD+1
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVGW(JWD,CDN(JD,JWUSP(JS))),JD=1,NACD(JWUSP(JS)))  
              end if
            ELSE  
            !  JSSS(JBUSP(JS))=JSSS(JBUSP(JS))+1
                qwdo(j)           = qwdo(j)            +qsp(js)                 ! cb 1/16/13  
                ...
                ENDIF
              WRITE(WDO2(JFILE,2),'(F8.2,F8.2)')JDAY,TAVG(JSSS(JBUSP(JS)),JBUSP(JS))  
              IF (CONSTITUENTS)then
                cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qsp(js)*cavg(jsss(jbusp(js)),jbusp(js),cn(1:nac))            ! cb 1/16/13
                WRITE (WDO2(JFILE,3),'(F8.2,1000F8.3)') JDAY,(CAVG(JSSS(JBUSP(JS)),JBUSP(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVG(JSSS(JBUSP(JS)),JBUSP(JS),CDN(JD,JWUSP(JS))),JD=1,NACD(JWUSP(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NPU  ! PUMP
          ...
          IF(IWDO(J) == IUPU(JS))THEN  
            JFILE=JFILE+1  
            IF(PUMPON(JS))THEN  
              WRITE(WDO2(JFILE,1),'(F8.2,F8.3)')JDAY,QPU(JS)  
            ELSE  
              WRITE(WDO2(JFILE,1),'(F8.2,F8.3)')JDAY,0.0  
            ENDIF  
            IF(LATERAL_PUMP(JS))THEN  
            !  JWD=JWD+1              
                IF(QPU(JS)==0.0)THEN
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVGW(JWD,CDN(JD,JWUPU(JS))),JD=1,NACD(JWUPU(JS)))  
              end if
            ELSE  
            !  JSSS(JBUPU(JS))=JSSS(JBUPU(JS))+1  
                IF(QPU(JS)==0.0)THEN
              ...
              WRITE(WDO2(JFILE,2),'(F8.2,F8.2)')JDAY,TAVG(JSSS(JBUPU(JS)),JBUPU(JS))  
              IF (CONSTITUENTS)then
                if(pumpon(js))then                ! cb 1/16/13
                  cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qpu(js)*cavg(jsss(jbupu(js)),jbupu(js),cn(1:nac)) 
                end if
                WRITE (WDO2(JFILE,3),'(F8.2,1000F8.3)') JDAY,(CAVG(JSSS(JBUPU(JS)),JBUPU(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                if(pumpon(js))then   ! cb 1/16/13
                  cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qpu(js)*cdavg(jsss(jbupu(js)),jbupu(js),cdn(1:nacd(jwupu(js)),jwupu(js))) 
                end if
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVG(JSSS(JBUPU(JS)),JBUPU(JS),CDN(JD,JWUPU(JS))),JD=1,NACD(JWUPU(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NPI  ! pipes
          ...
          IF(IWDO(J) == IUPI(JS))THEN  
            JFILE=JFILE+1  
            WRITE(WDO2(JFILE,1),'(F8.2,F8.3)')JDAY,QPI(JS)  
            IF(LATERAL_PIPE(JS))THEN  
           !   JWD=JWD+1  
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVGW(JWD,CDN(JD,JWUPI(JS))),JD=1,NACD(JWUPI(JS)))  
              end if
            ELSE  
            !  JSSS(JBUPI(JS))=JSSS(JBUPI(JS))+1
                qwdo(j)           = qwdo(j)            +qpi(js)     ! cb 1/16/13
                ...
                ENDIF
              WRITE(WDO2(JFILE,2),'(F8.2,F8.2)')JDAY,TAVG(JSSS(JBUPI(JS)),JBUPI(JS))  
              IF (CONSTITUENTS)then
                cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qpi(js)*cavg(jsss(jbupi(js)),jbupi(js),cn(1:nac))     ! cb 1/16/13
                WRITE (WDO2(JFILE,3),'(F8.2,1000F8.3)') JDAY,(CAVG(JSSS(JBUPI(JS)),JBUPI(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVG(JSSS(JBUPI(JS)),JBUPI(JS),CDN(JD,JWUPI(JS))),JD=1,NACD(JWUPI(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NGT  ! gates
          ...
          IF(IWDO(J) == IUGT(JS))THEN  
            JFILE=JFILE+1  
            WRITE(WDO2(JFILE,1),'(F8.2,F8.3)')JDAY,QGT(JS)  
            IF(LATERAL_GATE(JS))THEN  
          !    JWD=JWD+1  
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVGW(JWD,CDN(JD,JWUGT(JS))),JD=1,NACD(JWUGT(JS)))  
              end if
            ELSE  
           !   JSSS(JBUGT(JS))=JSSS(JBUGT(JS))+1
               qwdo(j)           = qwdo(j)            +qgt(js)     ! cb 1/16/13
               twdo(j)           = twdo(j)            +qgt(js)*tavg(jsss(jbugt(js)),jbugt(js))
                ...
                ENDIF
              WRITE(WDO2(JFILE,2),'(F8.2,F8.2)')JDAY,TAVG(JSSS(JBUGT(JS)),JBUGT(JS))  
              IF (CONSTITUENTS)then
                cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qgt(js)*cavg(jsss(jbugt(js)),jbugt(js),cn(1:nac))     ! cb 1/16/13
                WRITE (WDO2(JFILE,3),'(F8.2,1000F8.3)') JDAY,(CAVG(JSSS(JBUGT(JS)),JBUGT(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F8.2,1000F8.3)') JDAY,(CDAVG(JSSS(JBUGT(JS)),JBUGT(JS),CDN(JD,JWUGT(JS))),JD=1,NACD(JWUGT(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        ...
        DO JC=1,NAC  
          ...
          WRITE (CWDOC(CN(JC)),'(F8.3)') CWDO(CN(JC),J)                        ! SW 9/23/13 Changed format from G8.3 to F8.3 to avoid format overflow
        ...
        DO JD=1,NACD(JW)  
          ...
          WRITE (CDWDOC(CDN(JD,JW)),'(F8.3)') CDWDO(CDN(JD,JW),J)              ! SW 9/23/13 Changed format from G8.3 to F8.3 to avoid format overflow
          CDWDOC(CDN(JD,JW)) = ADJUSTR(CDWDOC(CDN(JD,JW)))  
        END DO  
        IF(JDAY<10000.)THEN  
          WRITE (WDO(J,1),'(F8.3,F8.2,8X,100F8.2)') JDAY, QWDO(J), (QOUTLET(I),I=1,NUMOUTLETS)  
          WRITE (WDO(J,2),'(F8.3,F8.2,8X,100F8.2)') JDAY, TWDO(J), (TOUTLET(I),I=1,NUMOUTLETS)  
        ...
        ELSE  
          WRITE (WDO(J,1),'(F8.2,F8.2,8X,100F8.2)') JDAY, QWDO(J), (QOUTLET(I),I=1,NUMOUTLETS)  
          WRITE (WDO(J,2),'(F8.2,F8.2,8X,100F8.2)') JDAY, TWDO(J), (TOUTLET(I),I=1,NUMOUTLETS)  
